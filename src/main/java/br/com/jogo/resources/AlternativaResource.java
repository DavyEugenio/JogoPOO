...
public class AlternativaResource {
	...
	public ResponseEntity<Void> update(@Valid @RequestBody AlternativaDTO objDto, @PathVariable Integer id) {
		jogo.updateAlternativa(objDto);
		return ResponseEntity.noContent().build();
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.DELETE)
	public ResponseEntity<Void> delete(@PathVariable Integer id) {
		jogo.deleteAlternativa(id);
		return ResponseEntity.noContent().build();
	}

	@RequestMapping(method = RequestMethod.GET)
	public ResponseEntity<List<AlternativaDTO>> findAll() {
		List<Alternativa> list = jogo.findAllQuestoes();
		List<AlternativaDTO> listDto = list.stream().map(obj -> new AlternativaDTO(obj)).collect(Collectors.toList());
